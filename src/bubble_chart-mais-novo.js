...
// Define as cores
  ...
  function createNodes(rawData) {
    ...
    var myNodes = rawData.map(function (d) {
      ...
      return {
        id: d.id,
        radius: ((d.destaque !== 'undefined') ? radiusScale(+d.lugares+4000) : radiusScale(+d.lugares)),
        value: +d.lugares,
        name: d.nome,
        name2: d.complemento,
...
// Visualização de Busca --------------------------------------------------------------------------------
  ...
  function buscaBubbles(buscaId,regiaoBuscaId,datavisMem) {

    console.log('buscaId no começo da busca: ' + regiaoBuscaId);
...
simulation.force("r", isolate(radialForceBusca, function(d) { 
  ...
  console.log('buscaId antes da bifurcação: ' + regiaoBuscaId);
  console.log('regiaoId antes da bifurcação: ' + regiaoId);

    if (regiaoBuscaId == "capital") {
        var regiaoMem = 'capital'
        console.log(' 1 ');
        showunidadeTitles();
        simulation.force('x', d3.forceX().strength(forceStrength).x(nodeunidadeXPos));
    ...
    } else if (regiaoBuscaId == "interior") {
      console.log(' 2 ');
